...
export default function page() {
	...
	useEffect(() => {
		axios
			.get(`${process.env.NEXT_PUBLIC_BACKEND_URL}/api/berita`)
			.then(function (res) {
		...
		<div>
				...
				<div className="mx-auto container">
					<div className="px-4  py-12 grid md:grid-cols-6">
						<div className="pb-2 max-w-xl  col-span-4">
							<h1 className="text-main tracking-wide font-semibold text-xl md:text-4xl leading-8 md:leading-12">
								Kabar & Informasi Terkini Jalan Tol Bali Mandara
							</h1>
						</div>
						<div className=" w-full col-span-2">
							<p className="leading-normal text-gray-500 text-md ">
								Baca berita terbaru tentang proyek, layanan,
								serta inovasi yang kami hadirkan untuk
								meningkatkan pengalaman berkendara Anda.{" "}
							</p>
						</div>
					</div>
				</div>
			...
			<section className="container mx-auto py-12">
				<div className="grid grid-cols-2 md:grid-cols-3 gap-4 mx-auto w-sm md:w-full ">
					{dataBerita.map((res: any, idx: number) => (
						<div
							className="flex flex-col md:w-96 w-full  mx-auto"
							key={idx}
						>
							<div className="">
								<img
									src={`${process.env.NEXT_PUBLIC_BACKEND_URL}/storage/image/berita/${res.cover}`}
									className="rounded-3xl w-full h-[200px] md:h-[250px]"
									alt=""
									srcSet=""
								/>
							</div>
							...
							<h1 className="mt-5 text-md md:text-xl tracking-wide font-semibold dark:text-gray-900">
								{res.judul.length > 100
									? `${res.judul.substring(0, 95) + "..."}`
									: res.judul}
							</h1>
							<div className="py-5">
								<a
									href={`/berita/${res.slug}`}
